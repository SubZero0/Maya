...
namespace Maya.Modules.Commands
{
    ...
    public class GeneralCommands : ModuleBase<MayaCommandContext>
    {
        ...
        public async Task Hi()
        {
            ...
            if (Context.MainHandler.GuildPersonalityHandler(Context.Guild).IsReady())
                own = Context.MainHandler.GuildPersonalityHandler(Context.Guild).GetOwnAnswers();
            else
                own = new string[0, 0];
            string[,] temp = Context.MainHandler.ConfigHandler.GetAnswers();
            ...
            await ReplyAsync(Utils.GetRandomWeightedChoice(sum));
        }
        ...
        public class TagModule : ModuleBase<MayaCommandContext>
        {
            ...
            public async Task Delete([Required] string tag = null)
            {
                ...
                Tag _tag = Context.MainHandler.GuildTagHandler(Context.Guild).GetTag(tag);
                if (!await Context.MainHandler.PermissionHandler.IsAtLeastAsync(Context.User, AdminLevel.MODERATOR) && _tag.creator != Context.User.Id)
                {
                    await ReplyAsync("This tag isn't yours.");
                    return;
                }
                IUser user = await Context.Guild.GetUserAsync(_tag.creator) ?? await Context.Client.GetUserAsync(_tag.creator);
                if (user != null && await Context.MainHandler.PermissionHandler.GetAdminLevelAsync(user) >= await Context.MainHandler.PermissionHandler.GetAdminLevelAsync(Context.User))
                {
                    await ReplyAsync("You can't delete a tag from a higher-ranking admin.");
                    return;
                }
            ...
            public async Task Edit([Required] string tag = null, [Required, Remainder] string text = null)
            {
                ...
                if (user != null && await Context.MainHandler.PermissionHandler.GetAdminLevelAsync(user) >= await Context.MainHandler.PermissionHandler.GetAdminLevelAsync(Context.User))
                {
                    await ReplyAsync("You can't edit a tag from a higher-ranking admin.");
                    return;
                }
    ...
    public class AdminCommands : ModuleBase<MayaCommandContext>
    {
        ...
        public async Task Forcesave()
        {
            ...
            await Context.MainHandler.DatabaseHandler.SaveAsync();
            await ReplyAsync("Saved tags, titles, and DB.");
        }

        [Command("reload")]
        [RequireAdmin(AdminLevel.ADMIN)]
        public async Task Reload([Required("shipcache/ships/answers/config/cookies/guild/localconfig/localdb/localmusic"), Remainder] string what = null)
        {
            switch (what)
            {
                case "shipcache":
                ...
                case "cookies":
                    {
                        await Context.MainHandler.DatabaseHandler.LoadAsync();
                        break;
                    }
                case "guild":
                    {
    ...
    {
        PaginationService _pagination;
        public WowsCommands(PaginationService pagination)
        {
        ...
        public async Task Ships()
        {
            ...
            var shipList = Context.MainHandler.ShipHandler.GetShipList();
            var ordShipList = shipList.OrderBy(x => x.GetName());

            List<PageBuilder> pages = new List<PageBuilder>();
            int tpages = (int)Math.Ceiling(shipList.Length / 30.0);
                ...
                List<EmbedFieldBuilder> fields = new List<EmbedFieldBuilder>
                    {
                        new EmbedFieldBuilder
                        {
                            IsInline = true,
                            Name = "​",
                            Value = $"​{String.Join("\n", ordShipList.Skip(i*30).Take(10).Select(x => x.GetName()))}" //zero-width-space he
                        },
                        new EmbedFieldBuilder
                        {
                            IsInline = true,
                            Name = "​",
                            Value = $"​{String.Join("\n", ordShipList.Skip(i*30+10).Take(10).Select(x => x.GetName()))}" //zero-width-space he
                        },
                        new EmbedFieldBuilder
                        {
                            IsInline = true,
                            Name = "​",
                            Value = $"​{String.Join("\n", ordShipList.Skip(i*30+20).Take(10).Select(x => x.GetName()))}" //zero-width-space here
                        }
                    };
                pages.Add(new PageBuilder { Fields = fields });
            }

            await _pagination.SendPaginatedMessageAsync(Context.Channel, new PaginatedMessage(pages, $"Ship list ({shipList.Length})", Utils.GetRandomColor()));
        }

        [Command("wtr")]
        [Summary("Show the WTR from a user")]
        public async Task Wtr([Required] string username = null, string region = null)
        {
            ...
            Utils.WoWSRegion _region = Utils.StringToWowsRegion(region);
            if (_region == Utils.WoWSRegion.Unknown)
                _region = Utils.GetUserWoWSRegion(Context.User);
            using (var typing = Context.Channel.EnterTypingState())
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    var jsonraw = await httpClient.GetStringAsync($"{_region.GetWowsApiUrl()}wows/account/list/?application_id=ca60f30d0b1f91b195a521d4aa618eee&type=startswith&limit=1&search={username}");
                ...
                try
                {
                    using (var image = new HttpClient())
                        using (var stream = await image.GetStreamAsync($"{_region.GetWtrUrl()}signature/{account_id}/light.png"))
                            await Context.Channel.SendFileAsync(stream, $"{account_id}.png", $"({_region.WoWSRegionToString()}) {account_name}");
                }
                catch (Exception)
                {
                    await ReplyAsync("Something went wrong while downloading the image.");
        ...
        [Remarks("Examples: stats Name NA North Carolina, stats Name North Carolina")]
        public async Task Stats([Required] string username = null, string regionOptional = null, [Remainder] string ship = null)
        {
            try //TODO: Remove when stable
            {
                if (Regex.Match(username, "^[a-zA-Z0-9_]{3,24}$") == Match.Empty)
                {
                    await ReplyAsync("Invalid username.");
                ...
                Utils.WoWSRegion _region = Utils.StringToWowsRegion(regionOptional ?? "");
                if (_region == Utils.WoWSRegion.Unknown)
                {
                    _region = Utils.GetUserWoWSRegion(Context.User);
                    if (ship != null && regionOptional != null)
                        ship = $"{regionOptional} {ship}";
                }
                using (var typing = Context.Channel.EnterTypingState())
                {
                    string account_id = null;
                    ...
                    if (ship == null)
                    {
                        ...
                        using (var httpClient = new HttpClient())
                        {
                            var jsonraw = await httpClient.GetStringAsync($"{_region.GetWowsApiUrl()}wows/account/info/?application_id=ca60f30d0b1f91b195a521d4aa618eee&account_id={account_id}");
                            JObject json = JObject.Parse(jsonraw);
                            if ((String)json["status"] != "ok")
                            {
                                await ReplyAsync("Something went wrong with the WoWS API.");
                        ...
                        EmbedBuilder eb = new EmbedBuilder();
                        eb.Title = $"Stats of {account_name} ({_region.WoWSRegionToString()})";
                        eb.Color = Utils.GetRandomColor();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            using (var httpClient = new HttpClient())
                            {
                                var jsonraw = await httpClient.GetStringAsync($"{_region.GetWowsApiUrl()}wows/ships/stats/?application_id=ca60f30d0b1f91b195a521d4aa618eee&account_id={account_id}&ship_id={r[0].GetId()}");
                                JObject json = JObject.Parse(jsonraw);
                                if ((String)json["status"] != "ok")
                                {
                                    await ReplyAsync("Something went wrong with the WoWS API.");
                            ...
                            EmbedBuilder eb = new EmbedBuilder();
                            eb.Title = $"Stats of {account_name}'s {r[0].GetName()} ({_region.WoWSRegionToString()})";
                            eb.Color = Utils.GetRandomColor();
                            if (result == null)
    ...
    [Name("Cookies")]
    [RequireContext(ContextType.Guild)]
    public class CookieCommands : ModuleBase<MayaCommandContext>
    {
        [Command("cookies")]
        [Alias("cookie")]
        [Summary("Show how many cookies someone has")]
        public async Task Cookies(IGuildUser user = null)
        {
            if (user == null)
                user = (IGuildUser)Context.User;
            await ReplyAsync("", embed: new EmbedBuilder().WithDescription($"🍪 {user.Mention} has {Context.MainHandler.DatabaseHandler.GetCookies(user.Id)} cookie(s)!"));
        }

        [Command("givecookie")]
        [Alias("givecookies")]
        [Summary("Give some of your cookies to someone")]
        public async Task Givecookies([Required] IGuildUser user = null, long amount = 1)
        {
            if (user == Context.User)
            {
                await ReplyAsync("You can't give cookies to yourself.");
                return;
            }
            if (user.IsBot)
            {
                await ReplyAsync("You can't give cookies to bots.");
                return;
            }
            if (amount < 1)
            {
                await ReplyAsync("You have to give at least 1 cookie.");
                return;
            }
            if (Context.MainHandler.DatabaseHandler.GetCookies(Context.User.Id) < amount)
            {
                await ReplyAsync("You don't have enough cookies.");
                return;
            }
            Context.MainHandler.DatabaseHandler.AddCookies(Context.User.Id, -amount);
            Context.MainHandler.DatabaseHandler.AddCookies(user.Id, amount);
            await ReplyAsync("", embed: new EmbedBuilder().WithDescription($"🍪 {Context.User.Mention} gave {amount} cookie(s) to {user.Mention}!"));
        }

        [Command("eat")]
        [Alias("eatcookie", "eatcookies", "nom", "nomcookie")]
        [Summary("Eat one of your cookies")]
        public async Task Eatcookie()
        {
            if (Context.MainHandler.DatabaseHandler.GetCookies(Context.User.Id) == 0)
            {
                await ReplyAsync("You don't have cookies.");
                return;
            }
            Context.MainHandler.DatabaseHandler.AddCookies(Context.User.Id, -1);
            await ReplyAsync("", embed: new EmbedBuilder().WithDescription($"<:blobnomcookie:305861483806326786> {Context.User.Mention} ate a cookie!"));
        }

        [Command("bake")]
        [Alias("bakecookie", "bakecookies")]
        [Summary("Bake cookies!")]
        public async Task Bakecookies()
        {
            DateTime? ts = Context.MainHandler.DatabaseHandler.GetLastBake(Context.User.Id);
            if (ts != null && ts > DateTime.UtcNow)
            {
                await ReplyAsync($"You need to wait {Utils.ToReadableString((ts.Value - DateTime.UtcNow))} to bake more cookies!");
                return;
            }
            string[,] bake =
            {
            ...
            int amount = int.Parse(Utils.GetRandomWeightedChoice(bake));
            Context.MainHandler.DatabaseHandler.UpdateLastBake(Context.User.Id);
            Context.MainHandler.DatabaseHandler.AddCookies(Context.User.Id, amount);
            await ReplyAsync("", embed: new EmbedBuilder().WithDescription($"🍪 {Context.User.Mention} baked {amount} cookie(s)!"));
        }
    }
    ...
    [Name("Fun")]
    public class FunCommands : ModuleBase<MayaCommandContext>
    {
        [Command("choose")]
